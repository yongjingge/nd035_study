...
public class CredentialService {
    ...
    public Integer addCredential (CredentialForm credentialForm) {
        ...
        Integer generatedID = credentialMapper.addCredential(newCredential);
        newCredential.setCredentialid(generatedID);
        return generatedID; // if added successfully into CredentialMapper, it will return an autogenerated credentialid.
    }

    // edit Credential
    public void updateCredential (CredentialForm credentialForm) {
        String encodedKey = this.generateEncodedKey();
        Credential target = credentialMapper.getCredentialById(credentialForm.getCredentialid());
        target.setUrl(credentialForm.getUrl());
        target.setUsername(credentialForm.getUsername());
        target.setKey(encodedKey);
        String encryptedPassword = encryptionService.encryptValue(credentialForm.getPassword(), encodedKey);
        target.setPassword(encryptedPassword);
        credentialMapper.updateCredential(target);
    }
